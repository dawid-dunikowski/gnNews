...
const ButtonsGrid = () =>{
    ...
    return (
      <div className="btn-group me-3 d-none d-md-inline-flex">
        <button type="button" className={`btn btn-outline-secondary waves-effect ${active?'active':''}`} onClick={()=>dispatch(switchViewNews(true))}>
        ...
        <button type="button" className={`btn btn-outline-secondary waves-effect ${!active?'active':''}`} onClick={()=>dispatch(switchViewNews(false))}>
