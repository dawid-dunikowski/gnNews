...
const ButtonsGrid = () =>{
    ...
    return (
      <div className="btn-group me-3 d-none d-md-inline-flex">
        <button type="button" aria-label="button-grid" className={`btn btn-outline-secondary waves-effect ${active?'active':''}`} onClick={()=>dispatch(switchViewNews(true))}>
        ...
        <button type="button" aria-label="button-list" className={`btn btn-outline-secondary waves-effect ${!active?'active':''}`} onClick={()=>dispatch(switchViewNews(false))}>
